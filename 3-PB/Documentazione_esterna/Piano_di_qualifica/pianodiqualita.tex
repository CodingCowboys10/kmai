...
\begin{xltabular}{\textwidth}{| l | X | c | c |}
    ...
    \hline
    MPC-13 & \ccgloss{Code Coverage} & 75\% & 100\% \\
    \hline
...
\begin{xltabular}{\textwidth}{| l | X | c | c |}
    ...
    \hline
    MPD-2 & \ccgloss{Requisiti} obbligatori soddisfatti & 100\% & 100\% \\
    \hline
...
Tali test, coerentemente a quanto prefigge il \ccgloss{V model} e il documento Norme\_di\_progetto\_v2.0, sono individuati parallelamente alle attività di sviluppo.\\
...
I seguenti test di accettazione hanno costituito la base per l'approvazione del \ccgloss{Minimum Viable Product}, verificando quali funzionalità sono state implementate nel prodotto. Essi sono stati definiti pensando ai test necessari per il collaudo di un prodotto finito.
...
\begin{xltabular}{\textwidth}{| c | X | X | c |}
    ...
    \endhead
    \hline
    TA-1 & Verificare che l’utente possa selezionare un modello di \ccgloss{LLM} con cui interagire. & UC-1,\newline UC-2 & \textcolor{cmarkcolor}{\ding{51}}  \\
    \hline
    TA-2 & Verificare che l’utente possa visualizzare i documenti presenti sul modello selezionato.
     & UC-3 &\textcolor{cmarkcolor}{\ding{51}}  \\
    \hline
    TA-3 & Verificare che l’utente possa visualizzare il nome,  la data di inserimento, la dimensione e il contenuto dei documenti.
     & UC-4.1,\newline UC-4.2,\newline UC-4.4,\newline UC-4.6 &\textcolor{cmarkcolor}{\ding{51}}  \\
    \hline
    TA-4 & Verificare che l’utente possa visualizzare i tag associati ai documenti.
     & UC-4.3 &\textcolor{cmarkcolor}{\ding{51}}  \\
    \hline
    TA-5 & Verificare che l’utente possa visualizzare lo stato di visibilità dei documenti.
     & UC-4.5 &\textcolor{cmarkcolor}{\ding{51}}  \\
    \hline
    TA-6 & Verificare che l’utente possa ricercare i documenti per nome. & UC-5.1 & \textcolor{cmarkcolor}{\ding{51}}  \\
    \hline
    TA-7 & Verificare che l’utente possa ricercare i documenti per data di inserimento. & UC-5.2 & \textcolor{cmarkcolor}{\ding{51}}  \\
    \hline
    TA-8 & Verificare che l’utente possa ricercare i documenti per tag associati. & UC-5.3 & \textcolor{xmarkcolor}{\ding{55}}  \\
    \hline
    ...
    \end{enumerate}& UC-12,\newline UC-12.1 & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    ...
    \end{enumerate}& UC-13,\newline UC-13.1,\newline UC-13.2 & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    ...
    \hline
    TA-16 & Verificare che il sistema mostri un messaggio d'errore nel caso venga tentato il caricamento di un file non supportato.& UC-14.2 & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    ...
    \end{enumerate}& UC-15,\newline UC-16 & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    
    TA-18 & Verificare che l’utente, prima di inviare domande al \ccgloss{chatbot}, possa:
    \begin{enumerate}
        \item visualizzare la lista delle lingue supportate;
        \item selezionare una lingua.
    \end{enumerate}& UC-17,\newline UC-18 & \textcolor{xmarkcolor}{\ding{55}} \\
    \hline
    TA-19 & Verificare che l’utente, quando deve inviare domande al chatbot, possa inserire la domanda in una barra di digitazione.& UC-19.1 & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    ...
    \end{enumerate}& UC-21,\newline UC-21.1,\newline UC-21.2,\newline UC-22 & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    ...
    \end{enumerate}& UC-28,\newline UC-28.1,\newline UC-28.2 & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TA-25 & Verificare che l'utente possa ascoltare la risposta ricevuta.& UC-29 & \textcolor{xmarkcolor}{\ding{55}} \\
    \hline
...
\begin{xltabular}{\textwidth}{| c | X | c | c |}
    ...
    \hline
    TS-64 & Verificare che il sistema effettui correttamente una ricerca semantica (semantic search) tra la domanda posta dall'utente e i gli \ccgloss{embedding} dei documenti caricati restituendo il documento, o pagina di esso, inerente alla domanda. & RFO-64 & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    ...
    \hline
    TS-71 & Verificare che il sistema permetta l'utilizzo di LLM tramite \ccgloss{OpenAI}. & RVO-1 & \textcolor{cmarkcolor}{\ding{51}}\\
    \hline
    ...
    \hline
    TS-73 & Verificare che il sistema permetta l'utilizzo di LLM locali tramite \ccgloss{Ollama}. & RVO-3 & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    ...
    \hline
    TS-76 & Verificare che il sistema processi correttamente file audio, tra i formati supportati, di dimensione fino a 5MB. & RVZ-15 & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
...
\begin{xltabular}{\textwidth}{| c | X | c |}
    ...
    \hline
    \endhead
    TU-1 & Verificare che il \ccgloss{componente} ChatForm venga renderizzato correttamente. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-2 & Verificare che il componente ChatList venga renderizzato correttamente. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-3 & Verificare che il componente ChatMessages venga renderizzato correttamente. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-4 & Verificare che il componente DeleteChatList venga renderizzato correttamente. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-5 & Verificare che il componente Message venga renderizzato correttamente. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-6 & Verificare che il componente ModelToggle venga renderizzato correttamente. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-7 & Verificare che il componente ThemeToggle venga renderizzato correttamente. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-8 & Verificare che il componente Settings venga renderizzato correttamente. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-9 & Verificare che il componente DataTable venga renderizzato correttamente. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-10 & Verificare che il componente DataTablePagination venga renderizzato correttamente. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-11 & Verificare che il componente DataTableViewOption venga renderizzato correttamente. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-12 & Verificare che il componente DocAction venga renderizzato correttamente. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-13 & Verificare che il componente DocActionDelete venga renderizzato correttamente. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-14 & Verificare che il componente DocForm venga renderizzato correttamente. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-15 & Verificare che il componente DocTable venga renderizzato correttamente. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-16 & Verificare che il componente Body venga renderizzato correttamente. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-17 & Verificare che il componente SideBar venga renderizzato correttamente. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-18 & Verificare che il componente ChangePageButton venga renderizzato correttamente. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-19 & Verificare che AddChatController restituisca una risposta con status 200 in caso di operazione con esito positivo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-20 & Verificare che AddChatMessagesController restituisca una risposta con status 200 in caso di operazione con esito positivo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-21 & Verificare che DeleteAllChatController restituisca una risposta con status 200 in caso di operazione con esito positivo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-22 & Verificare che DeleteChatController restituisca una risposta con status 200 in caso di operazione con esito positivo. & \textcolor{cmarkcolor}{\ding{51}}\\
    \hline
    TU-23 & Verificare che GetChatMessagesController restituisca una risposta con status 200 in caso di operazione con esito positivo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-24 & Verificare che GetChatsController restituisca una risposta con status 200 in caso di operazione con esito positivo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-25 & Verificare che AddDocumentController restituisca una risposta con status 200 in caso di operazione con esito positivo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-26 & Verificare che DeleteDocumentController restituisca una risposta con status 200 in caso di operazione con esito positivo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-27 & Verificare che UpdateDocumentController restituisca una risposta con status 200 in caso di operazione con esito positivo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-28 & Verificare che GetDocumentsController restituisca una risposta con status 200 in caso di operazione con esito positivo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-29 & Verificare che GetDocumentContentController restituisca una risposta con status 200 in caso di operazione con esito positivo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-30 & Verificare che AddChatController restituisca una risposta con status 500 in caso di operazione con esito negativo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-31 & Verificare che AddChatMessagesController restituisca una risposta con status 500 in caso di operazione con esito negativo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-32 & Verificare che DeleteAllChatController restituisca una risposta con status 500 in caso di operazione con esito negativo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-33 & Verificare che DeleteChatController restituisca una risposta con status 500 in caso di operazione con esito negativo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-34 & Verificare che GetChatMessagesController restituisca una risposta con status 500 in caso di operazione con esito negativo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-35 & Verificare che GetChatsController restituisca una risposta con status 500 in caso di operazione con esito negativo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-36 & Verificare che AddDocumentController restituisca una risposta con status 500 in caso di operazione con esito negativo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-37 & Verificare che DeleteDocumentController restituisca una risposta con status 500 in caso di operazione con esito negativo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-38 & Verificare che UpdateDocumentController restituisca una risposta con status 500 in caso di operazione con esito negativo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-39 & Verificare che GetDocumentsController restituisca una risposta con status 500 in caso di operazione con esito negativo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-40 & Verificare che GetDocumentContentController restituisca una risposta con status 500 in caso di operazione con esito negativo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-41 & Verificare che la server action addChat lanci un errore contenente il messaggio ricevuto dal \ccgloss{controller} in caso di operazione con esito negativo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-42 & Verificare che la server action addChatMessages lanci un errore contenente il messaggio ricevuto dal controller in caso di operazione con esito negativo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-43 & Verificare che la server action deleteAllChat lanci un errore contenente il messaggio ricevuto dal controller in caso di operazione con esito negativo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-44 & Verificare che la server action deleteChat lanci un errore contenente il messaggio ricevuto dal controller in caso di operazione con esito negativo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-45 & Verificare che la server action getChatMessages lanci un errore contenente il messaggio ricevuto dal controller in caso di operazione con esito negativo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-46 & Verificare che la server action getChats lanci un errore contenente il messaggio ricevuto dal controller in caso di operazione con esito negativo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-47 & Verificare che la server action addDocument lanci un errore contenente il messaggio ricevuto dal controller in caso di operazione con esito negativo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-48 & Verificare che la server action deleteDocument lanci un errore contenente il messaggio ricevuto dal controller in caso di operazione con esito negativo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-49 & Verificare che la server action updateDocument lanci un errore contenente il messaggio ricevuto dal controller in caso di operazione con esito negativo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-50 & Verificare che la server action getDocuments lanci un errore contenente il messaggio ricevuto dal controller in caso di operazione con esito negativo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-51 & Verificare che la server action getDocumentContent lanci un errore contenente il messaggio ricevuto dal controller in caso di operazione con esito negativo. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-52 & Verificare che il metodo getDocuments di DocumentRepository restituisca correttamente le informazioni sui documenti recuperati. & \textcolor{cmarkcolor}{\ding{51}} \\ 
    \hline
    TU-53 & Verificare che il metodo getDocumentContent di DocumentRepository restituisca correttamente i link di visualizzazione recuperati. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-54 & Verificare che il metodo getChatMessages di ChatRepository restituisca correttamente i messaggi della sessione recuperati. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-55 & Verificare che il metodo getChats di ChatRepository restituisca correttamente le informazioni sulle sessioni recuperate. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-56 & Verificare che il metodo getIdsEmbedding di EmbeddingRepository restituisca correttamente gli id degli embedding recuperati. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-57 & Verificare che il metodo addMessages di PostgresDataSource effettui le query corrette al \ccgloss{database} per salvare un nuovo messaggio. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-58 & Verificare che il metodo addOne di PostgresDataSource effettui le query corrette al database per creare una nuova sessione. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-59 & Verificare che il metodo deleteAll di PostgresDataSource effettui le query corrette al database per eliminare tutte le sessioni. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-60 & Verificare che il metodo deleteOne di PostgresDataSource effettui le query corrette al database per eliminare una sessione. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-61 & Verificare che il metodo getAll di PostgresDataSource effettui le query corrette al database per recuperare le sessioni. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-62 & Verificare che il metodo getAllMessages di PostgresDataSource effettui le query corrette al database per recuperare i messaggi di una sessione. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-63 & Verificare che il metodo addOne di MinioDataSource effettui correttamente putObject di AWS per inserire un documento. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-64 & Verificare che il metodo deleteOne di MinioDataSource effettui correttamente deleteObject di AWS per eliminare un documento. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-65 & Verificare che il metodo getAll di MinioDataSource effettui correttamente listObjects di AWS per recuperare i documenti. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-66 & Verificare che il metodo getContent di MinioDataSource effettui correttamente getObjectTagging di AWS per recuperare il link di visualizzazione di un documento. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-67 & Verificare che il metodo updateOne di MinioDataSource effettui correttamente putObjectTagging di AWS per aggiornare i tag di un documento. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-68 & Verificare che il metodo addOne di ChromaDataSource effettui correttamente add di ChromaClient per aggiungere dei nuovi embedding al database vettoriale. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-69 & Verificare che il metodo deleteOne di ChromaDataSource effettui correttamente delete di ChromaClient per eliminare degli embedding dal database vettoriale. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-70 & Verificare che il metodo updateOne di ChromaDataSource effettui correttamente update di ChromaClient per aggiornare i metadati associati a degli embedding nel database vettoriale. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline
    TU-71 & Verificare che il metodo getIds di ChromaDataSource effettui correttamente get di ChromaClient per recuperare gli id di alcuni embedding nel database vettoriale. & \textcolor{cmarkcolor}{\ding{51}} \\
    \hline

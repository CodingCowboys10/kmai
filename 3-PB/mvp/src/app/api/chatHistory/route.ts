...
export async function GET(req: NextRequest) {
    try {
        ...
        const query = 'SELECT id, message, role, model, date FROM ChatHistory ORDER BY date ASC';
...
export async function POST(req: NextRequest) {
    try {
        ...
        const result = await pool.query('SELECT * FROM ChatHistory WHERE id = $1', [messageId]);
        if (result.rows.length === 0) {
            await pool.query('INSERT INTO ChatHistory (id, message, role, model, date) VALUES ($1, $2, $3, $4, $5)', [messageId, messageContent, messageRole, messageModel, messageDate]);
        }
